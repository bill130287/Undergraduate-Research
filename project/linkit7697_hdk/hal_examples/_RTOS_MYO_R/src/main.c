...
#define MQTT_TOPIC  "MYTOPIC/right"  // right
#define MQTT_CLIENT_ID "7697_right"
...
void MQTT_output()
{
    ...
    while(1)
    {
        //To recognize you are handing up or not
        while(1)
        {
       MQTTYield(&c, 10);
            GetSensor();//index must be ZERO
       
            if(my_data[1][index]<-6000)
            {
                char buf[5];
                sprintf(buf, "up");
                message.payload = (void *)buf;
                message.payloadlen = strlen(buf) + 1;
                MQTTPublish(&c, topic2, &message);//*topic2 = "hand_up"
                break;
            }    
        }
        printf("You are handing up.################################\r\n");
