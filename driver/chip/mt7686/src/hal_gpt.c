...
hal_gpt_status_t hal_gpt_start_timer_ms(hal_gpt_port_t gpt_port, uint32_t timeout_time_ms, hal_gpt_timer_type_t timer_type)
{
    ...
    gp_gpt[gpt_port]->GPT_CON_UNION.GPT_CON_CELLS.EN &= ~GPT_COUNT_START;            /* stop timer */
    ...
    if (timer_type) {
        gp_gpt[gpt_port]->GPT_CON_UNION.GPT_CON = (uint32_t)(GPT_MODE_REPEAT_CTR | GPT_CLOCK_UNGATE_CTR | GPT_COUNT_START);
    } else {
        gp_gpt[gpt_port]->GPT_CON_UNION.GPT_CON = (uint32_t)(GPT_MODE_ONE_SHOT_CTR | GPT_CLOCK_UNGATE_CTR | GPT_COUNT_START);
    }
...
hal_gpt_status_t hal_gpt_start_timer_us(hal_gpt_port_t gpt_port, uint32_t timeout_time_us, hal_gpt_timer_type_t timer_type)
{
    ...
    /* set to mode, open clock source and start counter */
    if (timer_type) {
        gp_gpt[gpt_port]->GPT_CON_UNION.GPT_CON = (uint32_t)GPT_MODE_REPEAT | GPT_CLOCK_UNGATE | GPT_COUNT_START;
    } else {
        gp_gpt[gpt_port]->GPT_CON_UNION.GPT_CON = (uint32_t)GPT_MODE_ONE_SHOT | GPT_CLOCK_UNGATE | GPT_COUNT_START;
    }
...
hal_gpt_status_t hal_gpt_sw_get_remaining_time_ms(uint32_t handle, uint32_t *remaing_time)
{
    ...
    current_time = gpt_sw_context.absolute_time + gp_gpt[HAL_GPT_SW_PORT]->GPT_COUNT;
